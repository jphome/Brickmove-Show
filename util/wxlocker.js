(function(){
        ...
        wxlocker.prototype.storePass = function(psw,cb) {// touchend结束之后对密码和状态的处理
            if (this.checkPass(this.pswObj.spassword, psw)) {
                ...
                wx.setStorageSync('password_input', psw);
                cb();
            } else {
        ...
        wxlocker.prototype.init = function() {//初始化锁盘
            if (! wx.getStorageSync('passwordxx')) {
                console.log("初始化密码")
                wx.setStorageSync('passwordxx', JSON.stringify([{x:0,y:0,index:1},{x:0,y:0,index:4},{x:0,y:0,index:7},{x:0,y:0,index:8},{x:0,y:0,index:9}]));
            }
            this.pswObj = {
                spassword: JSON.parse(wx.getStorageSync('passwordxx'))
            };
            var password_input = wx.getStorageSync('password_input');
            if (password_input) {
                console.log("输入过密码,进行校对")
                this.unlockSucc = this.checkPass(this.pswObj.spassword, password_input);
                if (this.unlockSucc) {
                    return;
                }
            }
